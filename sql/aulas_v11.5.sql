...
-- Tiempo de generación: 29-02-2024 a las 01:13:43
...
-- Funciones
--
CREATE DEFINER=`root`@`localhost` FUNCTION `encrypt_password` (`input_password` VARCHAR(255)) RETURNS VARBINARY(255) DETERMINISTIC
BEGIN
...
CREATE TABLE `registro_salida` (
  ...
  `id_producto_salida` int(45) NOT NULL,
...
INSERT INTO `registro_salida` (`id_salida`, `id_producto_salida`, `unidades`, `id_factura_salidas`) VALUES
(601, 401, 2, 501),
(602, 408, 5, 502),
(603, 402, 1, 503),
(604, 406, 4, 504),
...
ALTER TABLE `registro_salida`
  ...
  ADD KEY `id_producto_salida` (`id_producto_salida`);
...
ALTER TABLE `registro_salida`
  ...
  ADD CONSTRAINT `registro_salida_ibfk_2` FOREIGN KEY (`id_producto_salida`) REFERENCES `producto` (`id_producto`);
